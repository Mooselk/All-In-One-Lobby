...
public class LobbyCommand implements CommandExecutor {
			...
			if (args.length == 0) {
				...
				if (args[0].equalsIgnoreCase("setspawn")) {
					if (p.hasPermission("lobby.setspawn")) {
						...
						p.sendMessage(ChatColor.translateAlternateColorCodes('&',  "&f[&6Lobby&f] Set spawn in world '&6"
								+ p.getWorld().getName()
								+ "&f' at X: &6"
								+ loc.getBlockX()
								+ "&f Y: &6"
				...
				if (args[0].equalsIgnoreCase("spawn")) {
					if (p.hasPermission("lobby.spawn")) {
						...
						p.sendMessage(ChatColor.translateAlternateColorCodes('&',  "&f[&6Lobby&f] Teleporting to spawn..."));
					} else {
						p.sendMessage(ChatColor.translateAlternateColorCodes('&',"You do not have permission to use this command."));
					}
				}
	...
	private void helpMessage(Player p) {
		p.sendMessage(
				ChatColor.translateAlternateColorCodes('&', "&m------------&r[ &l&6Lobby Help&r ]&m------------"));
		...
		p.sendMessage(
				ChatColor.translateAlternateColorCodes('&', "&6/lobby reload <config> &f- Reload specifed config."));
		p.sendMessage(ChatColor.translateAlternateColorCodes('&', ""));
	}
}