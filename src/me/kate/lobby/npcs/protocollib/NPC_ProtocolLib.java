package me.kate.lobby.npcs.protocollib;
...
import me.kate.lobby.npcs.protocollib.utils.MathHelper;
import me.kate.lobby.npcs.protocollib.wrapper.WrapperPlayServerEntityDestroy;
import me.kate.lobby.npcs.protocollib.wrapper.WrapperPlayServerPlayerInfo;
import me.kate.lobby.npcs.protocollib.wrapper.WrapperPlayServerScoreboardTeam;
...
public class NPC_ProtocolLib implements NPC {
    ...
    private static List<NPC_ProtocolLib> npcs = new ArrayList<NPC_ProtocolLib>();
    ...
    public static void startTask(Plugin plugin){
        if(!taskstarted){
            taskstarted = true;
            NPC_ProtocolLib.plugin = plugin;
            Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
                @Override
                public void run() {
                    for(NPC_ProtocolLib nmsnpc : npcs){
    ...
    public NPC_ProtocolLib(Location location, NPCSkin skin){
    ...
    private void spawnEnttity(Player p, GameProfile profile){
        ...
        try {
            ...
            Bukkit.getScheduler().runTaskLater(NPC_ProtocolLib.plugin, new Runnable() {
                @Override
                public void run() {
                    NPC_ProtocolLib.this.rmvFromTablist(p, profile);
                }
            }, 26L);
            this.rendered.add(p);
