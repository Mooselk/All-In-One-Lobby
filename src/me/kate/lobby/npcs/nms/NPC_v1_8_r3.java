...
import me.kate.lobby.npcs.event.NPCDespawnEvent;
import me.kate.lobby.npcs.event.NPCSpawnEvent;
import me.kate.lobby.npcs.skin.*;
import me.kate.lobby.npcs.utils.StringUtils;
...
public class NPC_v1_8_r3 implements NPC {
	...
	private static List<NPC_v1_8_r3> npcs = new ArrayList<NPC_v1_8_r3>();
	...
	public HashMap<Player, SkinData> player_cache = new HashMap<Player, SkinData>();
	...
	public static void startTask(Plugin plugin) {
		if (!taskstarted) {
			taskstarted = true;
			NPC_v1_8_r3.plugin = plugin;
			Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
				@Override
				public void run() {
					for (NPC_v1_8_r3 nmsnpc : npcs) {
	...
	private JsonSkinData getCachedSkin() {
		if (NPCApi.getCache() && this.skin.getSkinType() != SkinType.PLAYER) {
	...
	public NPC_v1_8_r3(Location location, NPCSkin skin) {
	...
	private void spawnEnttity(Player p) {
		...
		Bukkit.getScheduler().runTaskLater(NPC_v1_8_r3.plugin, new Runnable() {
			@Override
			public void run() {
				rmvFromTablist(p);
			}
		...
		NPCSpawnEvent event = new NPCSpawnEvent(p, (NPC) this);
	...
	private void destroy(Player p) {
		...
		try {
			...
			/*
			 * PacketPlayOutScoreboardTeam removescbpacket = new
			 * PacketPlayOutScoreboardTeam(); Field f =
			 * removescbpacket.getClass().getDeclaredField("a"); f.setAccessible(true);
			 * f.set(removescbpacket, profile.getName()); f.setAccessible(false); Field f2 =
			 * removescbpacket.getClass().getDeclaredField("h"); f2.setAccessible(true);
			 * f2.set(removescbpacket, 1); f2.setAccessible(false); ((CraftPlayer)
			 * p).getHandle().playerConnection.sendPacket(removescbpacket);
			 */
			...
			NPCDespawnEvent event = new NPCDespawnEvent(p, (NPC) this);
			Bukkit.getPluginManager().callEvent(event);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	private void addToTablist(Player p) {
		...
		PacketPlayOutPlayerInfo.PlayerInfoData data = packet.new PlayerInfoData(gameprofile, 1,
				WorldSettings.EnumGamemode.NOT_SET, CraftChatMessage.fromString("§8" + gameprofile.getName())[0]);
	...
	private void addToTablist(Player p, GameProfile profile) {
		PacketPlayOutPlayerInfo packet = new PacketPlayOutPlayerInfo();
		PacketPlayOutPlayerInfo.PlayerInfoData data = packet.new PlayerInfoData(profile, 1,
				WorldSettings.EnumGamemode.NOT_SET, CraftChatMessage.fromString("§8" + profile.getName())[0]);
		@SuppressWarnings("unchecked")
		List<PacketPlayOutPlayerInfo.PlayerInfoData> players = (List<PacketPlayOutPlayerInfo.PlayerInfoData>) getValue(
				packet, "b");
		players.add(data);
