...
public class SkinManager {
		...
		Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
			public void run() {
				...
				JsonObject jsonresponse = getJsonResponse(
						"https://api.mojang.com/users/profiles/minecraft/" + name + "?at=" + unixTime);
	...
	public static void getSkinFromMojangAsync(final Plugin plugin, final String identifier,
			final SkinDataReply skinreply) {
		Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
			public void run() {
							...
							if (uuid != null) {
								JsonObject jsonresponse = getJsonResponse(
										"https://sessionserver.mojang.com/session/minecraft/profile/" + uuid
												+ "?unsigned=false");
								if (jsonresponse != null && !jsonresponse.has("error")) {
									JsonObject prop = jsonresponse.getAsJsonArray("properties").get(0)
											.getAsJsonObject();
									String value = prop.get("value").getAsString();
									String signature = prop.get("signature").getAsString();
									skinreply.done(new SkinData(value, signature));
	...
	public static void getSkinFromMCAPIAsync(final Plugin plugin, final String identifier,
			final SkinDataReply skinreply) {
		Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
			public void run() {
				JsonObject jsonresponse = getJsonResponse("https://mcapi.de/api/user/" + identifier);
				if (jsonresponse != null && jsonresponse.get("result").getAsJsonObject().get("status").getAsString()
						.equalsIgnoreCase("Ok") && jsonresponse.get("premium").getAsBoolean()) {
	...
	public static void getSkinFromMineskinAsync(final Plugin plugin, final int mineskinid,
			final SkinDataReply skinreply) {
		Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
			public void run() {
